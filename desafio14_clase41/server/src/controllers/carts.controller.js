...
export const createCart = async (req, res) => {
    ...
    try {
        ...
        if(newCart) return res.status(201).send({ msg:'Carrito creado exitosamente', payload: newCart})
...
export const deleteProductInCart = async (req, res) => {
    ...
    try {
        ...
        if (deletedProduct) {
            const updatedCart = await cartService.getById(cid);
            res.send({ msg: `Producto con el ID ${pid} eliminado del carrito`, payload: updatedCart });
        } else {
