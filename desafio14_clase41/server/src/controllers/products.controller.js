...
export const createProduct = async (req, res) => {
  ...
  try {
    ...
    if(productPrice <= 0){
      return res.status(400).send({error: "El precio debe ser mayor a cero"})
    }
    if(typeof productTitle !== 'string' || productTitle.trim() === "" || productTitle.length > 60 || productTitle.length < 2){
      return res.status(400).send({error: "El Titulo no es valido"})
    }
  ...
  newPost
  ? res.status(201).send({msg: `Producto creado correctamente`, payload: newPost})
